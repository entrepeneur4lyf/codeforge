...
func NewWriteTool(lspClients map[string]*lsp.Client, permissions permissionService, history *HistoryService) BaseTool {
	return &writeTool{
...
func (w *writeTool) Run(ctx context.Context, call ToolCall) (ToolResponse, error) {
	...
	// Save new version
	w.history.SaveFileVersion(filePath, params.Content)
